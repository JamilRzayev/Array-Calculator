...
async def add_balance_cmd(message: types.Message):
    # Команда только для админа или для тестов
    try:
...
async def process_buy_subscription(callback: types.CallbackQuery, bot: Bot):
    ...
    async with async_session() as session:
        ...
        if not success:
            await callback.answer("❌ Ошибка при взаимодействии с VPN-сервером. Попробуйте позже.", show_alert=True)
            return

        # Списание средств только при успехе
